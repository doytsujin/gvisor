...
type CgroupRegistry struct {
	// lastHierarchyID is the id of the last allocated cgroup hierarchy. Must be
	// accessed through atomic ops.
	//
...
func (r *CgroupRegistry) NextHierarchyID() (uint32, error) {
	...
	return 0, fmt.Errorf("cgroup hierarchy ID overflow")
}
...
// (nil, false). FindHierarchy takes a reference on the returned FS, which is
// transferred to the caller.
func (r *CgroupRegistry) FindHierarchy(ctypes []CgroupControllerType) (*vfs.Filesystem, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
// Register registers the provided set of controller with the registry as a new
// hierarchy. If any controller is already registered, the function returns an
// error without modifying the registry.
func (r *CgroupRegistry) Register(hid uint32, cs []CgroupController) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if _, ok := r.hierarchies[hid]; ok {
		return fmt.Errorf("hierarchy ID %d already in use", hid)
	}
