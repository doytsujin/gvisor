...
var availableControllers = []kernel.CgroupControllerType{controllerCPU, controllerMemory}

// SupportedMountOptions is the set of supported mount options for cgroupfs.
var SupportedMountOptions = []string{"all", "cpu", "memory"}
...
func (fsType FilesystemType) GetFilesystem(ctx context.Context, vfsObj *vfs.VirtualFilesystem, creds *auth.Credentials, source string, opts vfs.GetFilesystemOptions) (*vfs.Filesystem, *vfs.Dentry, error) {
	...
	if _, ok := mopts["all"]; ok {
		delete(mopts, "all")
		wantControllers = availableControllers
	}
	...
	if len(mopts) != 0 {
		ctx.Warningf("cgroupfs.FilesystemType.GetFilesystem: unknown options: %v", mopts)
		return nil, nil, syserror.EINVAL
	}
	...
	if vfsfs, ok := r.FindHierarchy(wantControllers); ok {
	...
	// controllers first.
	if err := r.Register(hid, fs.kcontrollers); err != nil {
...
// dir implements kernfs.Inode for a generic cgroup resource
// controller directory. Specific controllers extend this to add their own functionality.
//
// +stateify savable
type dir struct {
	dirRefs
